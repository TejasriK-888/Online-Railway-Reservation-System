...
-- =======================================================
-- ORRS SQL DUMP WITH VIEWS, TRIGGER, SUBQUERIES & JOINS
-- =======================================================
...
/*
-- ====================================================
-- VIEW: Passenger Ticket Details (INNER JOIN Example)
-- ====================================================
-- Shows full ticket info per passenger using INNER JOIN
CREATE VIEW view_passenger_ticket_details AS
SELECT 
    p.pass_id,  -- Passenger ID
    CONCAT(p.pass_fname, ' ', p.pass_lname) AS full_name,
    p.pass_email,
    t.train_name,
    t.train_no,
    t.train_dep_stat,
    ...
    t.fare_payment_code,
    t.confirmation
FROM orrs_passenger p
INNER JOIN orrs_train_tickets t ON p.pass_email = t.pass_email;

-- ======================================================
-- VIEW: Passenger with or without ticket (LEFT JOIN)
-- ======================================================
-- Includes passengers even if they have not booked a ticket
CREATE VIEW view_all_passengers_with_ticket_status AS
SELECT 
    p.pass_id,
    CONCAT(p.pass_fname, ' ', p.pass_lname) AS full_name,
    p.pass_email,
    ...
    t.confirmation
FROM orrs_passenger p
LEFT JOIN orrs_train_tickets t ON p.pass_email = t.pass_email;

-- =====================================================
-- VIEW: Approved Tickets and Train Info (INNER JOIN)
-- =====================================================
-- Approved tickets with matching train details
CREATE VIEW view_approved_tickets_with_train_info AS
SELECT 
    tkt.ticket_id,
    tkt.pass_name,
    tkt.train_no,
    ...
    tr.route,
    tr.time AS departure_time,
    tkt.confirmation
FROM orrs_train_tickets tkt
INNER JOIN orrs_train tr ON tkt.train_no = tr.number
WHERE tkt.confirmation = 'Approved';

-- ==========================================
-- TABLE: Ticket Log (used by the trigger)
-- ==========================================
-- Log table for ticket insert events
CREATE TABLE orrs_ticket_log (
...
-- TRIGGER: Insert Log
-- ====================
-- Logs every ticket insert into the log table
DELIMITER $$

CREATE TRIGGER after_ticket_insert
AFTER INSERT ON orrs_train_tickets
FOR EACH ROW
BEGIN
  INSERT INTO orrs_ticket_log (ticket_id, pass_email, train_no, action)
  VALUES (NEW.ticket_id, NEW.pass_email, NEW.train_no, 'Ticket Booked');
END$$

DELIMITER ;

-- ===================================================
-- VIEW (Subquery): High Paying Passengers
-- ===================================================
-- Passengers who paid above average fare
CREATE VIEW view_high_paying_passengers AS
SELECT 
    pass_name,
    pass_email,
    train_name,
    train_fare
FROM orrs_train_tickets
WHERE CAST(train_fare AS UNSIGNED) > (
    SELECT AVG(CAST(train_fare AS UNSIGNED)) FROM orrs_train_tickets
);

-- =========================================================
-- VIEW (Subquery): Passengers Without Approved Tickets
-- =========================================================
-- List of passengers who never booked or have unapproved tickets
CREATE VIEW view_unconfirmed_passengers AS
SELECT 
    pass_fname,
    pass_lname,
    pass_email
FROM orrs_passenger
WHERE pass_email NOT IN (
    SELECT pass_email FROM orrs_train_tickets WHERE confirmation = 'Approved'
);

-- ================================
-- END OF SQL FILE WITH JOINS ✅
-- ================================
*/